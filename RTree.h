...
public:
    ...
    int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
               bool a_resultCallback(DATATYPE a_data, void *a_context, double Mi[2], double Ma[2]), void *a_context);
...
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
                       bool a_resultCallback(DATATYPE a_data, void *a_context, double Mi[2], double Ma[2]),
                       void *a_context) {
#ifdef _DEBUG
    for(int index=0; index<NUMDIMS; ++index)
    {
    ...
    else // This is a leaf node
    {
        for (int index = 0; index < a_node->m_count; ++index) {
            if (Overlap(a_rect, &a_node->m_branch[index].m_rect)) {
                ...
                double temp1 = a_node->m_branch[index].m_rect.m_min[0];
                double temp2 = a_node->m_branch[index].m_rect.m_min[1];

                double Mi[2] = {temp1, temp2};

                temp1 = a_node->m_branch[index].m_rect.m_max[0];
                temp2 = a_node->m_branch[index].m_rect.m_max[1];

                double Ma[2] = {temp1, temp2};

                // NOTE: There are different ways to return results.  Here's where to modify
                if (&a_resultCallback) {
                    ++a_foundCount;
                    if (!a_resultCallback(id, a_context, Mi, Ma)) {
                        return false; // Don't continue searching
                    }
                }
            }
